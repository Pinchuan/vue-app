define(function () {
    var Map = cesc.require("esri/map"),
        ...
        Circle = dojo.require('esri/geometry/Circle'),
        units = cesc.require("esri/units"),
    ...
    return {
        ...
        createNewPoint: function (graphicLayer,baseMap, x, y, iconUrl, pictureWidth, pictureHeight, item) {
            var pictureMarkerSymbol = new PictureMarkerSymbol(iconUrl, pictureWidth, pictureHeight);
        ...
        //当前定位标注
        addMarkSymbolByDistance: function (map, x, y) {
            var graLayer = new GraphicsLayer();
            var geometry = new Point(x, y);
            var circleGeometry = new Circle(geometry, {
                radius: 300,
            });
            var symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([160, 82, 45,0.25]), 1), new Color([0, 191, 255, 0.2]));
            var graphic = new Graphic(circleGeometry, symbol);
            graLayer.add(graphic);
            map.addLayer(graLayer, 1);
            return graLayer;
        },
        addPositionCircle: function (map, x, y, width) {
            var geometry = new Point(x, y);
            var offset = -width / 2;
            var markSymbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, width,
                new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,
                    new Color([255, 0, 0,0.25]), 1),
                new Color([0, 191, 255, 0.2]));
            var graphic = new Graphic(geometry, markSymbol);
            var graLayer = new GraphicsLayer();
            graLayer.add(graphic);
            map.addLayer(graLayer, 0);
            return graLayer;
        },
        //添加圆圈标注
        addCircleSymbol: function (map, x, y, width, color) {
            var geometry = new Point(x, y);
            var offset = -width / 2;
            var markSymbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, width,
                new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,
        ...
        addPoint: function (map, x, y, imgURL, attribute) {
            var pictureMarkerSymbol = new PictureMarkerSymbol(imgURL, 30, 30);
            var point = new Point(x, y);
            var graphic = new Graphic(point, pictureMarkerSymbol);
            graphic.attributes = attribute;
            map.graphics.add(graphic);
        },
        addSymbol: function (map, x, y, imgURL, attribute) {
            var pictureMarkerSymbol = new PictureMarkerSymbol(imgURL, 30, 30);
            var point = new Point(x, y);
            var graphic = new Graphic(point, pictureMarkerSymbol);
            graphic.attributes = attribute;
        ...
        //地址解析
        getLocationCode: function (lat, lng, cb) {
            var qqmapKey = "KRNBZ-AR6R5-TCOIE-Q3A4T-6WHBV-7IBRU";
            // 调用腾讯地图api获取地址
            // 由于跨域限制，需要用jsonp方式访问
            var url = "http://apis.map.qq.com/ws/geocoder/v1/?location=" + lat
                + "," + lng + "&key=" + qqmapKey
            ...
            $.ajax({
                ...
                success: function (ajaxResult) {
                    ...
                    // var addressComponent = ajaxResult.result.address_component;
                    var addressCode = ajaxResult.result;
                    cb(addressCode);
                }
            });
